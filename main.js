...
if (!opts['test']) {
  if (global.db) {
    setInterval(async () => {
      ...
      if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'DDR4Session'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])));
    }, 30 * 1000);
  }
}
...
let prekey = []
let directorio = readdirSync("./DDR4Session")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./DDR4Session/${files}`)
})
} 

function purgeSessionSB() {
try {
let listaDirectorios = readdirSync('./DDR4Session/');
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./DDR4Session/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./DDR4Session/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-') /*|| fileInDir.startsWith('app-') || fileInDir.startsWith('session-')*/
})
SBprekey = [...SBprekey, ...DSBPreKeys]
DSBPreKeys.forEach(fileInDir => {
unlinkSync(`./DDR4Session/${directorio}/${fileInDir}`)
})
}
})
...
const directories = ['./DDR4Session/', './DDR4Session/']
